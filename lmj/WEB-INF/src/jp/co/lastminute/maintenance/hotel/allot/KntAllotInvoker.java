...
public class KntAllotInvoker
{
	private KntAllotDbm dbm;
  private List productList;
  private final int CSVCOLCOUNT = 55;

  public KntAllotInvoker(){}

  public void init(KntAllotDbm dbm){
    this.dbm = dbm;
...
KntAllotUpdate.logs("SEQ" + produktKey);
    }
    it = null;
  }
  // PRODUCT_ALTPのデータの作成
  ...
  public void updateWebAltp(String filename){
    try{
      ...
      while((record = in.readLine()) != null){
        ...
        if(csvpart == null){
          KntAllotUpdate.logs("spritRecord fail:" + record);
          continue;
        }
        if(csvpart.length != CSVCOLCOUNT){
          KntAllotUpdate.logs("CSV Record illigal:" + record);
          KntAllotUpdate.logs("Count=" + csvpart.length);
          continue;
        }
        // PRODUCT_ALTPのチェック
...
KntAllotUpdate.logs(ex);
  ...
  private void checkWebAltp(ProductAltpBean prodct, String[] csvpart){
    ...
    webalt.setSupnbr(csvpart[3]);
    webalt.setCtrnbr(getIntValue(csvpart[4]));
    webalt.setCtrnbr1(getIntValue(csvpart[5]));
    webalt.setAgrAreaCd(csvpart[6]);
    webalt.setAltDat(getIntValue(csvpart[9]));
    webalt.setHaveAlt(getIntValue(csvpart[8]));
    webalt.setRoomType(csvpart[10]);
    webalt.setAltNum(getIntValue(csvpart[11]));
    webalt.setRoomCapa(csvpart[12]);
    webalt.setMaxNr(getIntValue(csvpart[13]));
    webalt.setMinNr(getIntValue(csvpart[14]));
    webalt.setMealCode(csvpart[16]);
    webalt.setLastDay(getIntValue(csvpart[34]));
    webalt.setStartDay(getIntValue(csvpart[35]));
    webalt.setAltsts(csvpart[38]);
    webalt.setSyoType(csvpart[39]);
    webalt.setAgtRoomTypeName(csvpart[49]);
    if(csvpart[2].length() == 0){
      webalt.setCanpaign("NONE");
    } else {
      webalt.setCanpaign(csvpart[2]);
    }
