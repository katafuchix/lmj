...
public class KntAllotDbm extends DataSourceMaker
{
  ...
  public void init(){
    ...
    } catch(Exception e){ KntAllotUpdate.logs(e); System.exit(3);}
  }

  public void release(){
    try {
      conn.close();
    } catch(SQLException e){ KntAllotUpdate.logs(e);}
  }

  protected boolean doExec(String query){
KntAllotUpdate.logs(query);
  ...
  public void updateWebRecord(WebAltpBean webalt){
    ...
    if(webalt.isInsertFlg()){
      ...
      sql = "INSERT INTO WEBALTP (SEQ_NO, PRODUCT_SEQ_NO, PRODUCT_ID, ALTDAT, "
          + "HAVEALT, ALLOTNUM, CAMPAIGN, MEAL_CODE, ALTSTS, SYO_TYPE, "
          + "SUPNBR, CTRNBR, CTRNBR_1, AGR_AREA_CD, AGT_CD, ROOM_TYPE, "
          + "ROOM_CAPA, MAX_NR, MIN_NR, AGTROOMTYPE, LAST_DAY, START_DAY, "
          + "PRICE, PRICE2, PRICE3, PRICE4, PRICE5, PRICEA1, "
      ...
      + webalt.getHaveAlt() + ","
      + webalt.getAltNum() + ","
      + setStringCol(webalt.getCanpaign()) + ","
      + setStringCol(webalt.getMealCode()) + ","
      + setStringCol(webalt.getAltsts()) + ","
      + setStringCol(webalt.getSyoType()) + ","
      + setStringCol(webalt.getSupnbr()) + ","
      + webalt.getCtrnbr() + ","
      + webalt.getCtrnbr1() + ","
      + setStringCol(webalt.getAgrAreaCd()) + ","
      + "\'KNT\',"
      + setStringCol(webalt.getRoomType()) + ","
      + setStringCol(webalt.getRoomCapa()) + ","
      + webalt.getMaxNr() + ","
      + webalt.getMinNr() + ","
      + "\'NONE\',"
      + webalt.getLastDay() + ","
